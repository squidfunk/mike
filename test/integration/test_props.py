...
class PropsTestCase(unittest.TestCase):
    ...
    def _add_version(
        ...
        with git_utils.Commit(branch, 'commit message') as commit:
            commit.add_file(
                git_utils.FileInfo(
                    os.path.join(deploy_prefix, 'versions.json'), all_versions.dumps()
                )
            )

    def _test_set_props(
        self,
        expected_versions=[versions.VersionInfo('1.0', properties={'hidden': True})],
        ...
        if expected_message:
            ...
            self.assertRegex(
                message,
                r'^Set properties for {}( in .*)? with mike \S+$'.format(
                    expected_versions[0].version
                ),
            )
...
class TestGetProp(PropsTestCase):
    ...
    def test_branch(self):
        ...
        assertOutput(self, ['mike', 'props', '-b', 'branch', '1.0', 'hidden'], 'true\n')
    ...
    def test_from_subdir(self):
        ...
        with pushd('sub'):
            ...
            assertOutput(
                self,
                ['mike', 'props', '1.0', 'hidden', '-F', '../zensical.toml'],
                'true\n',
            )
    ...
    def test_from_subdir_explicit_branch(self):
        ...
        with pushd('sub'):
            ...
            assertOutput(
                self,
                ['mike', 'props', '1.0', 'hidden', '-b', 'gh-pages', '-r', 'origin'],
                'true\n',
            )
    ...
    def test_deploy_prefix(self):
        ...
        assertOutput(
            self,
            ['mike', 'props', '1.0', 'hidden', '--deploy-prefix', 'prefix'],
            'true\n',
        )
    ...
    def test_diverged_remote(self):
        ...
        assertOutput(
            self,
            ['mike', 'props', '1.0', 'hidden', '--ignore-remote-status'],
            stdout='false\n',
            stderr='',
        )
        self.assertEqual(git_utils.get_latest_commit('gh-pages'), clone_rev)
    ...
    def test_get_and_set(self):
        ...
        assertOutput(
            self,
            ['mike', 'props', '1.0', 'hidden', '--set', 'dev=true'],
            stdout='',
            stderr=('error: cannot get and set properties at the ' + 'same time\n'),
            returncode=1,
        )
...
class TestSetProps(PropsTestCase):
    ...
    def test_set_string_prop(self):
        ...
        check_call_silent(['git', 'checkout', 'gh-pages'])
        self._test_set_props(
            expected_versions=[versions.VersionInfo('1.0', properties={'kind': 'dev'})]
        )
    ...
    def test_branch(self):
        ...
        assertPopen(['mike', 'props', '1.0', '--set', 'hidden=true', '-b', 'branch'])
    ...
    def test_from_subdir(self):
        ...
        os.mkdir('sub')
        with pushd('sub'):
            assertPopen(['mike', 'props', '1.0', '--set', 'hidden=true'], returncode=1)
            assertPopen(
                [
                    'mike',
                    'props',
    ...
    def test_commit_message(self):
        self._add_version()
        assertPopen(
            ['mike', 'props', '1.0', '--set', 'hidden=true', '-m', 'commit message']
        )
        check_call_silent(['git', 'checkout', 'gh-pages'])
    ...
    def test_diverged_remote(self):
        ...
        assertOutput(
            self,
            ['mike', 'props', '1.0', '--set', 'hidden=true'],
            stdout='',
            stderr=(
        ...
        assertPopen(
            ['mike', 'props', '1.0', '--set', 'hidden=true', '--ignore-remote-status']
        )
        self.assertEqual(git_utils.get_latest_commit('gh-pages^'), clone_rev)
...
class TestSetPropsOtherRemote(PropsTestCase):
    ...
    def test_explicit_branch(self):
        ...
        assertPopen(
            ['mike', 'props', '1.0', '--set', 'hidden=true', '-p', '-b', 'pages']
        )
    ...
    def test_explicit_remote(self):
        ...
        assertPopen(
            ['mike', 'props', '1.0', '--set', 'hidden=true', '-p', '-r', 'remote']
        )
        check_call_silent(['git', 'checkout', 'mybranch'])
        self._test_set_props()
        self._test_rev('mybranch')
