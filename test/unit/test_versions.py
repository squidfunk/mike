...
class TestVersionInfo(unittest.TestCase):
    def test_create(self):
        ...
        with self.assertRaisesRegex(ValueError, "^'..' is not a valid version$"):
            VersionInfo('..')
        with self.assertRaisesRegex(ValueError, "^'foo/bar' is not a valid version$"):
            VersionInfo('foo/bar')
        with self.assertRaisesRegex(ValueError, "^'foo/bar' is not a valid version$"):
        ...
        with self.assertRaisesRegex(ValueError, "^'foo/bar' is not a valid alias$"):
            VersionInfo('1.0', aliases=['foo/bar'])
        with self.assertRaisesRegex(ValueError, '^duplicated version and alias$'):
    ...
    def test_from_json(self):
        self.assertEqual(
            VersionInfo.from_json({'version': '1.0', 'title': '1.0', 'aliases': []}),
            VersionInfo('1.0'),
        )
    ...
    def test_to_json(self):
        v = VersionInfo('1.0')
        self.assertEqual(v.to_json(), {'version': '1.0', 'title': '1.0', 'aliases': []})

        v = VersionInfo('1.0', '1.0.0', ['latest'])
        self.assertEqual(
            v.to_json(), {'version': '1.0', 'title': '1.0.0', 'aliases': ['latest']}
        )
    ...
    def test_loads(self):
        self.assertEqual(
            VersionInfo.loads('{"version": "1.0", "title": "1.0", "aliases": []}'),
            VersionInfo('1.0'),
        )
    ...
    def test_dumps(self):
        v = VersionInfo('1.0')
        self.assertEqual(
            json.loads(v.dumps()), {'version': '1.0', 'title': '1.0', 'aliases': []}
        )
    ...
    def test_update(self):
        ...
        self.assertEqual(v, VersionInfo('1.0', '1.0.1', ['latest', 'greatest']))
