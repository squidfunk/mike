...
def add_git_arguments(parser, *, commit=True, deploy_prefix=True):
    ...
    git.add_argument('-r', '--remote', help='origin to push to (default: origin)')
    git.add_argument('-b', '--branch', help='branch to commit to (default: gh-pages)')
    ...
    if commit:
        ...
        git.add_argument(
            '--allow-empty', action='store_true', help='allow commits with no changes'
        )
    ...
    if deploy_prefix:
        git.add_argument(
            ...
            complete='directory',
            help=(
                'subdirectory within {branch} where '
...
def add_set_prop_arguments(parser, *, prefix=''):
    ...
    prop_p.add_argument(
        '--{}set-string'.format(prefix),
        metavar='PROP=STRING',
        action='append',
        type=jsonpath.parse_set,
        dest='set_props',
        help='set the property at PROP to a STRING value',
    )
    prop_p.add_argument(
        '--{}set-all'.format(prefix),
        metavar='JSON',
        action='append',
    ...
    prop_p.add_argument(
        ...
        dest='set_props',
        help='delete the property at PROP',
    )
    prop_p.add_argument(
        '--{}delete-all'.format(prefix),
        action='append_const',
        const=('', jsonpath.Deleted),
        dest='set_props',
        help='delete all properties',
    )
    return prop_p
    ...
    try:
        ...
        if args.branch is None or args.remote is None:
            raise FileNotFoundError(
                '{}; pass --config-file or set --remote/--branch explicitly'.format(
                    str(e)
                )
            )
...
def list_versions(parser, args):
    def print_version(info):
        ...
        aliases = (
            ' [{}]'.format(', '.join(sorted(info.aliases))) if info.aliases else ''
        )
        if info.title != version:
            print(
                '"{title}" ({version}){aliases}'.format(
...
def main():
    ...
    parser.add_argument('-q', '--quiet', action='store_true', help='silence warnings')
    parser.add_argument(
        '--debug',
        action='store_true',
    ...
    deploy_p = subparsers.add_parser(
        'deploy', description=deploy_desc, help='build docs and deploy them to a branch'
    )
    deploy_p.set_defaults(func=deploy)
    deploy_p.add_argument(
        '-t', '--title', help='short descriptive title for this version'
    )
    deploy_p.add_argument(
        '-u',
        '--update-aliases',
    ...
    deploy_p.add_argument(
        ...
        help=(
            'method for creating aliases (one of: ' + '%(choices)s; default: symlink)'
        ),
    )
    deploy_p.add_argument(
        '-T', '--template', complete='file', help='template file to use for redirects'
    )
    add_git_arguments(deploy_p)
    add_set_prop_arguments(deploy_p, prefix='prop-')
    deploy_p.add_argument(
        'version', metavar='VERSION', help='version to deploy this build to'
    )
    deploy_p.add_argument(
        'aliases', nargs='*', metavar='ALIAS', help='additional alias for this build'
    )
    ...
    delete_p.add_argument('--all', action='store_true', help='delete everything')
    ...
    alias_p.add_argument(
        'identifier', metavar='IDENTIFIER', help='existing version or alias'
    )
    alias_p.add_argument('aliases', nargs='*', metavar='ALIAS', help='new alias to add')

    props_p = subparsers.add_parser(
        'props',
        description=props_desc,
        help='get/set version properties',
    ...
    props_p.add_argument(
        'get_prop', nargs='?', metavar='PROP', default='', help='property to get'
    )

    retitle_p = subparsers.add_parser(
        'retitle', description=retitle_desc, help='change the title of a version'
    )
    retitle_p.set_defaults(func=retitle)
    add_git_arguments(retitle_p)
    retitle_p.add_argument(
        'identifier', metavar='IDENTIFIER', help='version or alias to retitle'
    )
    ...
    list_p.add_argument(
        '-j', '--json', action='store_true', help='display the result as JSON'
    )
    ...
    set_default_p = subparsers.add_parser(
        'set-default',
        description=set_default_desc,
        help='set the default version for your docs',
    )
    set_default_p.set_defaults(func=set_default)
    set_default_p.add_argument(
        '-T', '--template', complete='file', help='template file to use'
    )
    set_default_p.add_argument(
        '--allow-undefined',
        action='store_true',
        help=('allow setting undefined versions as ' + 'default'),
    )
    add_git_arguments(set_default_p)
    set_default_p.add_argument(
        'identifier', metavar='IDENTIFIER', help='version or alias to set as default'
    )
    ...
    serve_p.add_argument(
        ...
        metavar='HOST[:PORT]',
        help=('Host address and port to serve from ' + '(default: %(default)s)'),
    )
    ...
    completion_p = subparsers.add_parser(
        'generate-completion',
        description=generate_completion_desc,
        help='print shell completion script',
    )
    completion_p.set_defaults(func=generate_completion)
    shell = os.path.basename(os.environ['SHELL']) if 'SHELL' in os.environ else None
